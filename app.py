...
class NorwayElectionSimulator:
    ...
    def __init__(self, use_threshold: bool = False):
        # Note: 4% threshold only applies to national leveling seats, not district seats
        self.use_threshold = use_threshold
        self.threshold = 0.04 if use_threshold else 0.0
    ...
    def modified_sainte_lague_allocation(self, votes: Dict[str, int], total_seats: int) -> Dict[str, int]:
        ...
        """
        # Filter out parties below threshold (only applies if use_threshold=True)
        ...
        if self.use_threshold:
            ...
            # For district seats, all parties with votes can compete
            qualified_parties = {party: vote_count for party, vote_count in votes.items() if vote_count > 0}
...
def streamlit_app():
    ...
    # Option to include threshold (for educational purposes)
    use_threshold = st.sidebar.checkbox(
        "Apply 4% threshold", 
        value=False,
        help="Note: In Norway's system, the 4% threshold only applies to national leveling seats, not district seats"
    )
    
    simulator = NorwayElectionSimulator(use_threshold=use_threshold)
    ...
    with col2:
        ...
        **Modified Sainte-Laguë Method:**
        ...
        - {'4% threshold applied' if use_threshold else 'No threshold (realistic for district seats)'}
